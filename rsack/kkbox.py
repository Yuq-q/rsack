...
class Download():
    ...
    def get_img_url(self, url_template, size, file_type='jpg'):
        ...
        urlshow = url
        urlshow = urlshow.replace('{width}', str(1000))
        urlshow = urlshow.replace('{height}', str(1000))
        logger.info("Downloading Cover: " + urlshow)
        if size > 2048:
    ...
    def _download_cover(self):
        ...
        url = self.get_img_url(
            self.meta['album']['album_photo_info']['url_template'], 6000)
        r = self.client.session.get(url)
        self.cover_path_embed = os.path.join(self.album_path, 'cover.jpg')
    ...
    def _download_album(self, id):
        ...
        logger.info("Release Date: " + self.meta['release_date'])
        logger.info("Album Url: " + self.meta['album']['album_more_url_s'])
        ...
        # Assign track total because the existing 'collected_count' is not accurate.
        self.meta['album']['track_total'] = song_list['song_list']['song'][-1]['trankno']
    ...
    def _download_track(self, song: dict):
        ...
        if file_ext == '.aac':  # Correct .aac to .m4a
            file_ext = '.m4a'
        file_name = sanitize(
            f"{song['trankno'].zfill(2)}. {song['text']}{file_ext}")
        ...
        if not os.path.exists(file_path):
            ...
            if song['song_lyrics_valid'] == 1:
                ...
                lyrics = ''
            Tag(file_path=file_path, file_ext=file_ext, track_meta=song,
                lyrics=lyrics, album_meta=self.meta, cover_path=self.cover_path_embed)
        else:
...
class Tag():
    ...
    def aac(self):
        ...
        audio['trkn'] = [(int(self.track_meta['trankno']), int(
            self.album_meta['album']['track_total']))]
    ...
    def flac(self):
        ...
        # Look into this
        audio['ALBUMARTIST'] = self.album_meta['album']['artist_name']
